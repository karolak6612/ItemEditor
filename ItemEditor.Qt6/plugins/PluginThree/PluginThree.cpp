...
PluginThree::PluginThree(QObject* parent)
    : BasePlugin(parent)
    , m_datParser(nullptr)
    , m_sprParser(nullptr)
{
    m_name = "Plugin Three";
    m_version = "1.0.0";
    m_supportedVersions << "10.00" << "10.10" << "10.20" << "10.30" << "10.31" << "10.32" 
                        << "10.33" << "10.34" << "10.35" << "10.36" << "10.37" << "10.38" 
                        << "10.39" << "10.40" << "10.41" << "10.50" << "10.51" << "10.52" 
                        << "10.53" << "10.54" << "10.55" << "10.56" << "10.57" << "10.58" 
                        << "10.59" << "10.60" << "10.61" << "10.62" << "10.63" << "10.64" 
                        << "10.65" << "10.66" << "10.67" << "10.68" << "10.69" << "10.70" 
                        << "10.71" << "10.72" << "10.73" << "10.74" << "10.75" << "10.76" 
                        << "10.77";
}

PluginThree::~PluginThree()
{
    cleanup();
}

bool PluginThree::initialize()
{
    QMutexLocker locker(&m_mutex);
    
    try {
        // Initialize DAT parser
        m_datParser = new DatParserV10();
        if (!m_datParser) {
            qWarning() << "PluginThree: Failed to create DAT parser";
            return false;
        }
        
        // Initialize SPR parser
        m_sprParser = new SprParserV10();
        if (!m_sprParser) {
            qWarning() << "PluginThree: Failed to create SPR parser";
            delete m_datParser;
            m_datParser = nullptr;
            return false;
        }
        
        qDebug() << "PluginThree: Successfully initialized for client versions 10.00-10.77";
        return true;
        
    } catch (const std::exception& e) {
        qCritical() << "PluginThree: Exception during initialization:" << e.what();
        cleanup();
        return false;
    }
}

QString PluginThree::name() const
{
    return m_name;
}

QString PluginThree::version() const
{
    return m_version;
}

QStringList PluginThree::supportedVersions() const
{
    return m_supportedVersions;
}

bool PluginThree::loadClient(const QString& datPath, const QString& sprPath)
{
    QMutexLocker locker(&m_mutex);
    ...
    quint32 datSignature = static_cast<DatParserV10*>(m_datParser)->getDatSignature();
    QString datVersion = static_cast<DatParserV10*>(m_datParser)->getClientVersion();
    ...
    quint32 sprSignature = static_cast<SprParserV10*>(m_sprParser)->getSprSignature();
    QString sprVersion = static_cast<SprParserV10*>(m_sprParser)->getClientVersion();
    ...
    if (!m_supportedVersions.contains(datVersion)) {
        emit errorOccurred(QString("Client version %1 is not supported by Plugin Three").arg(datVersion));
        cleanup();
        return false;
    }
    ...
    qDebug() << "PluginThree: Successfully loaded client version" << m_currentClientVersion 
             << "from" << datPath << "and" << sprPath;
    qDebug() << "PluginThree: DAT signature:" << QString::number(datSignature, 16).toUpper()
             << "SPR signature:" << QString::number(sprSignature, 16).toUpper();
    
    return true;
}

QByteArray PluginThree::getClientData(quint16 clientId)
{
    QMutexLocker locker(&m_mutex);
...
QByteArray PluginThree::getSpriteHash(quint16 clientId)
{
    QMutexLocker locker(&m_mutex);
...
QByteArray PluginThree::getSpriteSignature(quint16 clientId)
{
    QMutexLocker locker(&m_mutex);
...
bool PluginThree::isClientLoaded() const
{
    QMutexLocker locker(&m_mutex);
...
QString PluginThree::getClientVersion() const
{
    QMutexLocker locker(&m_mutex);
    return m_currentClientVersion;
}

void PluginThree::cleanup()
{
    QMutexLocker locker(&m_mutex);
    ...
    qDebug() << "PluginThree: Cleanup completed";
}

void PluginThree::clearCaches()
{
...
QByteArray PluginThree::calculateSpriteHash(const DatData& datData)
{
...
QByteArray PluginThree::calculateSpriteSignature(const DatData& datData)
{
...
QByteArray PluginThree::applyFFT2DRGB(const QByteArray& rgbData, int width, int height, bool reorder)
{
...
QByteArray PluginThree::calculateEuclideanDistanceSignature(const QByteArray& fftData, int width, int height, int blockSize)
{
...
double PluginThree::compareSpriteSignatures(const QByteArray& signature1, const QByteArray& signature2)
{
