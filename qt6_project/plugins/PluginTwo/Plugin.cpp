/**
 * Item Editor Qt6 - PluginTwo Implementation
...
Plugin::Plugin(QObject *parent)
    ...
    , m_loaded(false)
{
    qDebug() << "PluginTwo: Constructor called";
}
...
Plugin::~Plugin()
{
    ...
    qDebug() << "PluginTwo: Destructor called";
}
...
{
    qDebug() << "PluginTwo: Loading client" << client.description();
    ...
    if (!loadDat(datFullPath, client, extended, frameDurations)) {
        qDebug() << "PluginTwo: Failed to load dat file:" << datFullPath;
        return false;
    }

    if (!loadSprites(sprFullPath, client, extended, transparency)) {
        qDebug() << "PluginTwo: Failed to load spr file:" << sprFullPath;
        return false;
    }

    m_loaded = true;
    emit clientLoaded(client);
    qDebug() << "PluginTwo: Client loaded successfully";
    return true;
}

void Plugin::initialize()
{
    qDebug() << "PluginTwo: Initializing plugin";
    
    if (m_settings->load("PluginTwo.xml")) {
        m_supportedClients = m_settings->getSupportedClientList();
        qDebug() << "PluginTwo: Loaded" << m_supportedClients.size() << "supported clients";
    } else {
        qDebug() << "PluginTwo: Failed to load settings, creating default supported clients";
        
        // Create some default supported clients for demonstration
        ItemEditor::SupportedClient client1(800, "Tibia 8.00", 0, 0x4E119CC1, 0x4E119CC1);
        ItemEditor::SupportedClient client2(860, "Tibia 8.60", 0, 0x4E119CC2, 0x4E119CC2);
...
{
    qDebug() << "PluginTwo: Loading sprites from" << filename;
    ...
    if (!ItemEditor::Sprite::loadSprites(filename, m_sprites, client, extended, transparency)) {
        qDebug() << "PluginTwo: Failed to load sprite data from" << filename;
        return false;
    }
    
    qDebug() << "PluginTwo: Loaded" << m_sprites.size() << "sprites, now populating ClientItem bitmaps";
    ...
    for (auto it = m_items->begin(); it != m_items->end(); ++it) {
        ...
        if (!bitmap.isNull()) {
            ...
            if (processedItems <= 5) {
                qDebug() << QString("PluginTwo: Generated bitmap for item %1, size %2x%3, sprites: %4")
            ...
            if (itemsWithSprites <= 10) {
                qDebug() << QString("PluginTwo: Failed to generate bitmap for item %1 with %2 sprites")
    ...
    qDebug() << QString("PluginTwo: Successfully generated %1 valid bitmaps out of %2 items with sprites")
...
{
    qDebug() << "PluginTwo: Loading DAT from" << filename;
    
    QFile file(filename);
    if (!file.open(QIODevice::ReadOnly)) {
        qDebug() << "PluginTwo: Cannot open DAT file:" << filename;
        return false;
    }
    ...
    if (client.datSignature() != datSignature) {
        qDebug() << QString("PluginTwo: Bad dat signature. Expected %1, got %2")
    ...
    qDebug() << "PluginTwo: Loading" << m_itemCount << "items";
    ...
    for (quint16 id = 100; id <= m_itemCount; ++id) {
        ...
        // Read item flags (simplified - includes PluginTwo specific flags)
        quint8 flag;
        do {
            stream >> flag;
        ...
        for (quint32 i = 0; i < numSprites; ++i) {
            ...
            if (m_sprites.contains(spriteId)) {
                ...
                if (id <= 105) { // Only log first few items to avoid spam
                    qDebug() << QString("PluginTwo: Added sprite %1 to item %2, sprite list size: %3")
    ...
    emit loadingProgress(100);
    qDebug() << "PluginTwo: DAT loading completed";
    return true;
}
